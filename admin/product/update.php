...
<body>
    ...
    <?php foreach ($result as $row) : ?>
        ...
        <form method="POST" class="form" style="padding: 25px;" enctype="multipart/form-data">
            <h2>Sửa địa điểm</h2>
            <label>Image: <img style="width: 100px" src="../../images/product/<?php echo $row['image'] ?>" class="fix-label" name="old_image"></label><br>
            ...
            <label>Guest: <input type="text" value="<?php echo $row['guest'] ?>" class="fix-label" name="guest"></label><br>
            <label>Bed: <input type="text" value="<?php echo $row['bed'] ?>" class="fix-label" name="bed"></label><br>
            <label>Bathroom: <input type="text" value="<?php echo $row['bathroom'] ?>" class="fix-label" name="bathroom"></label><br>
            <label>Price: <input type="text" value="<?php echo $row['price'] ?>" class="fix-label" name="price"></label><br>
            ...
            <label style="align-items: center; display: flex;">Description:
                <textarea style="width: 450px; height: 150px; margin-bottom: 5px;" name="description"><?php echo $row['description'] ?></textarea>
            </label><br>
    ...
    if (isset($_POST['update'])) {
        ...
        if (!empty($_FILES["image"]["name"])) {
            // Xóa ảnh cũ
            $oldImage = $row['image'];
            $oldImagePath = "../../images/product/" . $oldImage;
            unlink($oldImagePath);

            // Xử lý ảnh mới
            $targetDirectory = "../../images/product/"; // Đường dẫn thư mục lưu trữ ảnh
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath)) {
                $sql = "UPDATE `products` SET name='$name', guest='$guest', bed='$bed', bathroom='$bathroom', price='$price', title='$title', short_des='$short_des', description='$description', image='$targetFileName' WHERE id='$id'";
            } else {
            ...
            $sql = "UPDATE `products` SET name='$name', guest='$guest', bed='$bed', bathroom='$bathroom', price='$price', title='$title', short_des='$short_des', description='$description' WHERE id='$id'";
        }
