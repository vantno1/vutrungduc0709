...
<body>
    ...
    <?php foreach ($result as $row) : ?>
        ...
        <form method="POST" class="form" style="padding: 25px;" enctype="multipart/form-data">
            <h2>Sửa địa điểm</h2>
            <label>Country: <input type="text" value="<?php echo $row['country'] ?>" class="fix-label" name="country"></label><br>
            <label>Place: <input type="text" value="<?php echo $row['place'] ?>" class="fix-label" name="place"></label><br>
            <label>Short Description: <input type="text" value="<?php echo $row['short_des'] ?>" class="fix-label" name="short_des"></label><br>
            <label>Price: <input type="number" value="<?php echo $row['price'] ?>" class="fix-label" name="price"></label><br>
            <label>Old Price: <input type="number" value="<?php echo $row['old_price'] ?>" class="fix-label" name="oprice"></label><br>
            <label>Current Image: <img style="width: 100px" src="../../images/places/<?php echo $row['image'] ?>" class="fix-label" name="old_image"></label><br>
            <label>Image: <input type="file" class="fix-label" name="image"></label><br>
    ...
    if (isset($_POST['update'])) {
        ...
        if (!empty($_FILES["image"]["name"])) {
            // Xóa ảnh cũ
            $oldImage = $row['image'];
            $oldImagePath = "../../images/places/" . $oldImage;
            unlink($oldImagePath);

            // Xử lý ảnh mới
            $targetDirectory = "../../images/places/"; // Đường dẫn thư mục lưu trữ ảnh
            ...
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFilePath)) {
                $sql = "UPDATE `places` SET country='$country', place='$place', short_des='$short_des', description='$des', price='$price', old_price='$oprice', image='$targetFileName' WHERE id='$id'";
            } else {
            ...
            $sql = "UPDATE `places` SET country='$country', place='$place', short_des='$short_des', description='$des', price='$price', old_price='$oprice' WHERE id='$id'";
        }
